...
function ContractContextProvider(props) {
    ...
    const Services = {
        ...
        verify_document: async (_documentId, _address) => {
            try {
                const verified = await state.DeDocs.methods.verify_document(Number(_documentId), _address).call()
                console.log(verified)
                return { success: true, data: { verified } }
            } catch (err) {
                console.log("Error in verifying document: ", err)
                return { success: false, message: err.message }
            }
        }
